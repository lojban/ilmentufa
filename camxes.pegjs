...
BEhO_elidible = expr:(BEhO_clause?) {return (expr == "") ? ["BEhO"] : _node_empty("BEhO", expr);}
BOI_elidible = expr:(BOI_clause?) {return (expr == "") ? ["BOI"] : _node_empty("BOI", expr);}
CU_elidible = expr:(CU_clause?) {return (expr == "") ? ["CU"] : _node_empty("CU", expr);}
DOhU_elidible = expr:(DOhU_clause?) {return (expr == "") ? ["DOhU"] : _node_empty("DOhU", expr);}
FEhU_elidible = expr:(FEhU_clause?) {return (expr == "") ? ["FEhU"] : _node_empty("FEhU", expr);}
// FOI and FUhO are never elidible
GEhU_elidible = expr:(GEhU_clause?) {return (expr == "") ? ["GEhU"] : _node_empty("GEhU", expr);}
KEI_elidible = expr:(KEI_clause?) {return (expr == "") ? ["KEI"] : _node_empty("KEI", expr);}
KEhE_elidible = expr:(KEhE_clause?) {return (expr == "") ? ["KEhE"] : _node_empty("KEhE", expr);}
KU_elidible = expr:(KU_clause?) {return (expr == "") ? ["KU"] : _node_empty("KU", expr);}
KUhE_elidible = expr:(KUhE_clause?) {return (expr == "") ? ["KUhE"] : _node_empty("KUhE", expr);}
KUhO_elidible = expr:(KUhO_clause?) {return (expr == "") ? ["KUhO"] : _node_empty("KUhO", expr);}
// LEhU is never elidible
LIhU_elidible = expr:(LIhU_clause?) {return (expr == "") ? ["LIhU"] : _node_empty("LIhU", expr);}
LOhO_elidible = expr:(LOhO_clause?) {return (expr == "") ? ["LOhO"] : _node_empty("LOhO", expr);}
LUhU_elidible = expr:(LUhU_clause?) {return (expr == "") ? ["LUhU"] : _node_empty("LUhU", expr);}
MEhU_elidible = expr:(MEhU_clause?) {return (expr == "") ? ["MEhU"] : _node_empty("MEhU", expr);}
NUhU_elidible = expr:(NUhU_clause?) {return (expr == "") ? ["NUhU"] : _node_empty("NUhU", expr);}
SEhU_elidible = expr:(SEhU_clause?) {return (expr == "") ? ["SEhU"] : _node_empty("SEhU", expr);}
TEhU_elidible = expr:(TEhU_clause?) {return (expr == "") ? ["TEhU"] : _node_empty("TEhU", expr);}
TOI_elidible = expr:(TOI_clause?) {return (expr == "") ? ["TOI"] : _node_empty("TOI", expr);}
TUhU_elidible = expr:(TUhU_clause?) {return (expr == "") ? ["TUhU"] : _node_empty("TUhU", expr);}
VAU_elidible = expr:(VAU_clause?) {return (expr == "") ? ["VAU"] : _node_empty("VAU", expr);}
VEhO_elidible = expr:(VEhO_clause?) {return (expr == "") ? ["VEhO"] : _node_empty("VEhO", expr);}
